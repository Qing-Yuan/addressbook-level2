...
If no person matches the keyword, a list of suggestions will be returned checking
part of the contact names for the keyword.
In the event that one keyword matches a name while the other matches part of the name, only
the name that matches the keyword perfectly is returned.
====
...
* `find J t` + 
Returns Any person having names containing `J` and `t`.

* `find John t` + 
Returns John only if such a contact exists, else any person having names containing `John` and `t`.
